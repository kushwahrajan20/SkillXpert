...
export const getPublishedCourses = async (_, res) => {
    try {
        ...
        return res.status(200).json({
            courses,
            message: "Courses displayed which are published"
        })
...
export const removeLecture = async (req,res) =>{
    try {
        ...
        //remove the lecture referencefrom the associated course
        await Course.updateOne(
            {lectures:lectureId},//find the course that contains the
...
export const togglePublishCourse = async(req,res)=>{
    try {
        ...
        console.log(error);
        return res.status(500).json({ message: "Failed to update course publish status" });
    }
}